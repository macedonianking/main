...
void main_money_find_test() {
	struct main_money_count buffer[] = { { 25, 0 }, { 10, 0 }, { 5, 0 },
			{ 1, 0 } };
	...
	for (i = 0; i < 1000; ++i) {
		...
		fprintf(stdout, "%d=", i);
		is_first = true;
		for (ptr = buffer, end = buffer + ARRAY_SIZE(buffer); ptr != end;
				++ptr) {
			if (0 == ptr->count) {
				continue;
			}
...
void main_find_maximum_linear_subarray_normal(const int *ptr, int n, int *l,
		int *r, int *v) {
	int i, j;
	int max, maxL, maxR;
	int sum;
...
static int main_find_maximum_linear_subarray_divide_impl(int *ptr, int s, int e,
		int *l, int *r, int *v) {
	int m;
	int outL, outR, outV;
	int maxL, maxR, max;
	...
	maxL = maxR = -1;
	max = -1;
	if (!(main_find_maximum_linear_subarray_divide_impl(ptr, s, m, &outL, &outR,
			&outV))) {
		if (maxL == -1 || max < outV) {
			maxL = outL;
			maxR = outR;
	...
	if (!main_find_maximum_linear_subarray_divide_impl(ptr, m + 1, e, &outL,
			&outR, &outV)) {
		if (maxL == -1 || max < outV) {
			maxL = outL;
			maxR = outR;
 ...
 * 分治法
 */
void main_find_maximum_linear_subarray_divide(int *ptr, int n, int *l, int *r,
		int *v) {
	if (n > 0) {
...
void main_find_maximum_linear_subarray_literal(const int *ptr, int n, int *l,
		int *r, int *s) {
	int outL, outR, outS;
...
void main_test_heap_sort() {
	...
	int buffer1[] = { 27, 17, 3, 16, 13, 10, 1, 5, 7, 12, 4, 8, 9, 0 };
	int n;
	int *buffer;
...
void main_test_quick_sort() {
	...
	// 快速排序
	n = 10;
	ptr = main_random_integer_buffer(n, F_MAIN_TEST_BUFFER_FM,
	F_MAIN_TEST_BUFFER_TO);
